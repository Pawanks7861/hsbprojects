...
function get_pur_invoices_by_id($id)
{
    $CI = &get_instance();

    $CI->db->select('invoice_number, vendor_invoice_number');
    $CI->db->where('id', $id);

    $result = $CI->db->get(db_prefix() . 'pur_invoices')->row();

    if ($result) {
        return $result->invoice_number . ' - ' . $result->vendor_invoice_number;
    }
