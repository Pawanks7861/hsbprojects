...
class Purchase_model extends App_Model
{
    ...
    public function get_purorder_pdf_html($pur_order_id)
    {
      ...
      <br><br>
      ';
        $order_summary_with_break = $pur_order->order_summary;


        $html .=  '<div style="page-break-after:always">' . $order_summary_with_break . '<br><b>Payment Terms: </b>' . get_payment_term_name($pur_order->payment_days) . '</div>

        <table class="table purorder-item bor" style="width: 100%">
        <thead>
          <tr>
            <th class="thead-dark" style="width: 12%">' . _l('items') . '</th>
        ...
        foreach ($pur_order_detail as $row) {
            ...
            $html .= '<tr nobr="true" class="sortable">
            <td style="width: 12%">' . $items->commodity_code . ' - ' . $items->description . '</td>
            ...
            <td align="right" style="width: 10%">' . $row['quantity']  . '<br>' . $unit_name . '</td>
        ...
        $html .= '<tr id="round_odd">
                 ...
                 <td class="subtotal">
                    ' . '₹ ' . app_format_money($pur_order->shipping_fee, '') . '
                 </td>
              </tr>';
        $html .= '<tr id="subtotal">
                 <td width="33%"></td>
    ...
    public function get_hsnsac_codes($id = '')
    {
        $this->db->order_by('id', 'desc');
        if ($id != '') {
            $this->db->where('id', $id);
            return $this->db->get(db_prefix() . 'hsn_sac_code')->row();
        } else {
            return $this->db->get(db_prefix() . 'hsn_sac_code')->result_array();
        }
    }

    public function add_hsncode($data)
    {
        $this->db->insert(db_prefix() . 'hsn_sac_code', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            return $insert_id;
        }
    ...
    public function update_hsncode($data, $id)
    {
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'hsn_sac_code', $data);
        if ($this->db->affected_rows() > 0) {
            return true;
        }
        return false;
    ...
    public function delete_hsncode($id)
    {
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'hsn_sac_code');
        if ($this->db->affected_rows() > 0) {
            return true;
        }
        return false;
    ...
    public function get_wo_order_detail($pur_request)
    {
        ...
        $wo_order_details = $this->db->get(db_prefix() . 'wo_order_detail')->result_array();

        foreach ($wo_order_details as $key => $detail) {
            $wo_order_details[$key]['discount_money'] = (float) $detail['discount_money'];
            $wo_order_details[$key]['into_money'] = (float) $detail['into_money'];
            $wo_order_details[$key]['total'] = (float) $detail['total'];
            $wo_order_details[$key]['total_money'] = (float) $detail['total_money'];
            $wo_order_details[$key]['unit_price'] = (float) $detail['unit_price'];
            $wo_order_details[$key]['tax_value'] = (float) $detail['tax_value'];
        }

        return $wo_order_details;
    }
    ...
    public function add_wo_order($data)
    {
        ...
        $cron_email['options'] = json_encode($cron_email_options, true);
        $this->db->insert(db_prefix() . 'cron_email', $cron_email);
        $this->save_purchase_files('work_order', $insert_id);
        if ($insert_id) {
            // Update next purchase order number in settings
            $next_number = $data['number'] + 1;
            ...
            // warehouse module hook after work order add
