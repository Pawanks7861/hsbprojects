...
$custom_fields = get_custom_fields('wo_order', [
    'show_on_table' => 1,
]);
...
$aColumns = [
    ...
    'total_tax',
    'total',
    '(SELECT GROUP_CONCAT(name SEPARATOR ",") FROM ' . db_prefix() . 'taggables JOIN ' . db_prefix() . 'tags ON ' . db_prefix() . 'taggables.tag_id = ' . db_prefix() . 'tags.id WHERE rel_id = ' . db_prefix() . 'wo_orders.id and rel_type="wo_order" ORDER by tag_order ASC) as tags',
    'approve_status',
    'delivery_date',
    'delivery_status',
    'number',
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == 'total') {
            ...
            if ((has_permission('work_order', '', 'edit') || is_admin()) && $aRow['approve_status'] != 2) {
                $numberOutput .= ' | <a href="' . admin_url('purchase/wo_order/' . $aRow['id']) . '">' . _l('edit') . '</a>';
            }
            if (has_permission('work_order', '', 'delete') || is_admin()) {
        ...
        } elseif ($aColumns[$i] == '(SELECT GROUP_CONCAT(name SEPARATOR ",") FROM ' . db_prefix() . 'taggables JOIN ' . db_prefix() . 'tags ON ' . db_prefix() . 'taggables.tag_id = ' . db_prefix() . 'tags.id WHERE rel_id = ' . db_prefix() . 'wo_orders.id and rel_type="wo_order" ORDER by tag_order ASC) as tags') {
