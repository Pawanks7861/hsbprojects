...
class purchase extends AdminController
{
    ...
    public function setting()
    {
        ...
        $data['hsnsac_codes'] = $this->purchase_model->get_hsnsac_codes();
    ...
    public function get_purchase_order_row_template()
    {
        ...
        echo $this->purchase_model->create_purchase_order_row_template($name, $item_name, $item_description, $quantity, $unit_name, $unit_price, $taxname, $item_code, $unit_id, $tax_rate, '', $discount, '', '', '', '', '', $item_key, false, $currency_rate, $to_currency, $hsn_code);
    }
    ...
    public function hsncode()
    {
        if ($this->input->post()) {
            ...
            $data = $this->input->post();
            if (!$this->input->post('id')) {
                $id = $this->purchase_model->add_hsncode($data);
                if ($id) {
                    $success = true;
                    $message = _l('added_successfully', _l('hsn_code'));
                    set_alert('success', $message);
                }
                redirect(admin_url('purchase/setting?group=hsnsac_codes'));
            } else {
                $id = $data['id'];
                unset($data['id']);
                $success = $this->purchase_model->update_hsncode($data, $id);
                if ($success) {
                    $message = _l('updated_successfully', _l('hsn_code'));
                    set_alert('success', $message);
                }
                redirect(admin_url('purchase/setting?group=hsnsac_codes'));
            }
    ...
    public function delete_hsncode($id)
    {
        ...
        $response = $this->purchase_model->delete_hsncode($id);
        if (is_array($response) && isset($response['referenced'])) {
            set_alert('warning', _l('is_referenced', _l('hsn_code')));
        } elseif ($response == true) {
            set_alert('success', _l('deleted', _l('hsn_code')));
        } else {
            set_alert('warning', _l('problem_deleting', _l('hsn_code')));
        }
        redirect(admin_url('purchase/setting?group=hsnsac_codes'));
    }

    public function pur_payments()
    {
        $this->load->view('payments/manage', $data);
    }
    public function table_pur_payments()
    {
    ...
    {
        if (!has_permission('record_payment', '', 'view') && !is_admin() && !has_permission('record_payment', '', 'view_own')) {
            access_denied('purchase');
        }
        $this->load->model('expenses_model');
        $this->load->model('payment_modes_model');
        ...
        $this->load->view('record_payment/manage', $data);
    }

    public function add_record_payment($id = '')
    {
        if ($this->input->post()) {
            $pur_order_data = $this->input->post();
            if ($id == '') {
